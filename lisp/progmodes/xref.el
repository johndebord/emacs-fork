...
(defun xref--show-xref-buffer (xrefs alist)
  (let ((xref-alist (xref--analyze xrefs))                                           ;; jd
        (xref-buffer (get-buffer-create xref-buffer-name)))                          ;; jd
      (setq buffer-undo-list nil)
      (let ((inhibit-read-only t)
            (buffer-undo-list t))
        (pop-to-buffer xref-buffer)                                                  ;; jd
        (xref--xref-buffer-mode)
        (erase-buffer)                                                               ;; jd
        (xref--insert-xrefs xref-alist)                                              ;; jd
        (goto-char (point-min))
        (setq xref--original-window (assoc-default 'window alist)
              xref--original-window-intent (assoc-default 'display-action alist))))) ;; jd
  ...
  (let (matches)
    ...
    (while (and
      ...
      (let* ((beg-column (- (match-beginning 0) line-beg))
             ...
             (summary (buffer-substring-no-properties line-beg line-end))) ;; jd
        (add-face-text-property beg-column end-column 'highlight           ;; jd
                                t summary)
